...
// --------------------------
// Serve Sitemap with BASE_DOMAIN as argument
// --------------------------
function serveSitemap(baseDomain) {
  return async function(req, res) {
    ...
    try {
      ...
      // Replace localhost URLs with the correct BASE_DOMAIN
      const sitemapUrls = campaign.sitemapUrls.map((url) => {
      ...
      const sitemapXml = generateSitemapXml(sitemapUrls, campaignId);
      res.set("Content-Type", "application/xml");
      res.send(sitemapXml);
    } catch (error) {
      console.error(error);
